...
class ParallelCoordinatesRenderer(Renderer):
    ...
    def as_json(self):
        ...
        trace = {"type": "parcoords", "dimensions": []}
        color_n = None
        for n, (label, values) in enumerate(tabular_dict.items()):
        ...
        if color_n is not None:
            color_column = trace["dimensions"].pop(color_n)
            trace["dimensions"].append(color_column)
